...
%   {\item[$\checkmark$] I previously built the Processor In the Loop (PIL) solution for a semiconductor customer}
   ...
   {\item[$\checkmark$] Hired Applied's vehicle platform team from scratch}
   {\item[$\checkmark$] Interviewed senior robotics, vehicle hardware, controls, and planning candidates (beyond GenSWE)}
   {\item[$\checkmark$] Player/coach: chief architect for XIL; reviews and hands-on coding as needed}
   \end{itemize}
\end{itemize}
