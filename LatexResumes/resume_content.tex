...
%\center{\it{Research engineer passionate about applying AI to the real world}}
\center{\it{Hands on Engineering and technical leader passionate about robotics and AI for the real world}}
%}{}
...
\textbullet~Systems Software & \textbullet~Hardware In Loop & \textbullet~RFID \\
...
\vspace{8pt}
{\sl \href{http://applied.co}{\textbf{Applied Intuition}}} \hfill        January 2020 - Present \\
\href{http://applied.co}{\textbf{Mountain View, California}}       \hfill   \textbf{Engineering Manager}
   ...
   \begin{itemize}
   {\item[$\checkmark$] Founding member of XIL: took Object HIL product from concept to production in 1 year}
	{\item[$\checkmark$] Managed a distributed team of engineers based in Mountain View, Austin and Munich}
	{\item[$\checkmark$] Team consisted of experts in C++ development, real time systems, GPU programming, and simulation}
   {\item[$\checkmark$] Have led the product vision and technical roadmap from the founding of the team}   
   {\item[$\checkmark$] My team has built Applied Intuition's Object HIL, Sensor HIL and Replay HIL products}
   {\item[$\checkmark$] I previously built the Model In the Loop (MIL) toolbox for the Applied Development Platform (ADP)} 
%   {\item[$\checkmark$] I previously built the Processor In the Loop (PIL) solution for a semiconductor customer}
   {\item[$\checkmark$] I previously designed a virtual ECU (vECU) simulator for software-in-the-loop (SIL) testing}
   {\item[$\checkmark$] I previously worked on our core-simulator engine and interfaces for Autonomous Driving/ADAS systems validation}
   \end{itemize}
   \item Responsible for overall architecture, product direction and engineering design
   \begin{itemize}
   {\item[$\checkmark$] Highly collaborative in working style - my team has worked across almost all engineering teams at Applied Intuition}
   {\item[$\checkmark$] Helped hire Applied's vehicle platform team from scratch}
   {\item[$\checkmark$] Interviewed most senior robotics, vehicle hardware and controls/planning roles beyond GenSWEs}
   {\item[$\checkmark$] Player/coach - managed engineers, am chief architect for XIL, do code-reviews and am hands-on in the codebase as needed}
   \end{itemize}
\end{itemize}


\vspace{8pt}
{\sl \href{http://artificial.com}{\textbf{Artificial}}} \hfill        September 2019 - November 2019 \\
\href{http://artificial.com}{\textbf{Palo Alto, California}}       \hfill   \textbf{Consultant}
   \begin{itemize} \itemsep -2pt % reduce space between items
   \item Helped team hone in on product-market fit for their Robotic Lab Automation
   \begin{itemize}
   {\item[$\checkmark$] Worked with product team and CTO, and ideated on product features and architecture}
	{\item[$\checkmark$] Implemented an initial simulator using three.js to visualize Robotic Lab workcells}
	{\item[$\checkmark$] Team took my prototype and it has become a core part of their product allowing customers to understand how their assays will be automated}
   \end{itemize}
\end{itemize}
...
\href{https://web.archive.org/web/20230130130556/https://level.ai/}{\textbf{Palo Alto, California}}       \hfill   \textbf{Founder, CTO}
   ...
   \begin{itemize}
   {\item[$\checkmark$] Invented algorithms that are at the core of our products (3 issued, 2 pending patents)}
...
%\vspace{0.2in}
% TODO: @arvindpereira to fix this section. It is not clear enough at the moment.
% \newpage
